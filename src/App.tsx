  ...
  return (
    <div style={{ display: 'flex', height: '100vh', overflow: 'hidden' }}>
      ...
      <div style={{ width: 360, padding: 16, overflowY: 'auto', borderRight: '1px solid #ddd' }}>
        ...
        {crates.map(c=>{
          ...
          return (
            <fieldset key={c.id} style={{marginBottom:16}}>
              ...
              <button
                style={{marginTop:8,background:'#d22',color:'#fff',border:'none',borderRadius:4,padding:'4px 10px',cursor:'pointer'}}
                onClick={() => setCrates(prev => prev.filter(crate => crate.id !== c.id))}
              >
                Delete
              </button>
            </fieldset>
        ...
        <button onClick={()=>{
          snapshot();
          addCrateFromData({label:`Crate ${crates.length+1}`,length:1,width:1,height:1,weight:50});
        }}>
          + Add blank crate
        </button>
