...
export default function App() {
  ...
  const addCrateFromData = (data: ParsedRow) => {
    if (overWeight) return alert("Truck load capacity exceeded.");
    setCrates(prev => [
      ...prev,
      {
        id: prev.length + 1,
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', overflow: 'hidden' }}>
      {(overflowIds.length > 0 || overWeight) && (
        <div style={{ position:'absolute', top:0, left:0, right:0, padding:'6px 12px', background: overWeight ? '#ff9800' : '#c62828', color:'#000', fontWeight:600, zIndex:50, textAlign:'center' }}>
          {overflowIds.length > 0 && <>⚠️ Truck space exceeded: {overflowIds.map(id => crates.find(c => c.id === id)!.label).join(', ')}</>}
          {overWeight && <> ⚠️ Truck load exceeded! Max: {maxLoad} kg, Current: {totalWeight} kg</>}
        </div>
      )}
      ...
      <div style={{ width: 360, padding: 16, overflowY: 'auto', borderRight: '1px solid #ddd' }}>
        ...
        {(['length','width','height'] as const).map(d=>(
          <p key={d}>{d}: <input type="number" style={{width:70}} value={truck[d]} onChange={e=>updTruck(d,Number(e.target.value))}/> {truck.unit}</p>
        ))}
        <p>Unit <select value={truck.unit} onChange={e=>updTruck('unit',e.target.value as Unit)}><option value="m">m</option><option value="cm">cm</option></select></p>
        <p>Max Load (kg): <input type="number" value={maxLoad} onChange={e=>setMaxLoad(Number(e.target.value))} /></p>

        <h3 style={{marginTop:24}}>Bulk import</h3>
        <input type="file" accept=".xls,.xlsx" onChange={handleFile} />
        <p style={{fontSize:12,color:'#666',margin:'4px 0'}}>Template: <a href="/Template.xlsx" download>download</a></p>
        ...
        {parsedRows.length > 0 && (
          <div style={{ border:'1px solid #ccc', padding:8, margin:'8px 0' }}>
            ...
            <table style={{ width:'100%', fontSize:12 }}>
              ...
              <tbody>{parsedRows.map((r,i)=>(
                <tr key={i}>
                  <td><input type="checkbox" checked={rowSelection.has(i)} onChange={e=>{ const s=new Set(rowSelection); e.target.checked? s.add(i): s.delete(i); setRowSelection(s); }}/></td>
                  <td>{r.label}</td><td>{r.height}</td><td>{r.length}</td><td>{r.width}</td><td>{r.weight}</td>
                </tr>
              ))}</tbody>
            </table>
            <button onClick={addSelectedRows} disabled={rowSelection.size===0} style={{ marginTop:6 }}>Add selected rows to plan</button>
          </div>
        )}

        <h2 style={{ marginTop: 24 }}>Crates</h2>
        {crates.map(c=>{
          const availableBases = crates.filter(b=>b.id!==c.id && !occupiedBaseIds.includes(b.id) && !b.stackTargetId);
          return (
            <fieldset key={c.id} style={{marginBottom:16}}>
              ...
              {(['height','length','width'] as const).map(dim=>(
                <p key={dim}>{dim}: <input type="number" style={{width:60}} value={c[dim]} onChange={e=>updCrate(c.id,{[dim]:Number(e.target.value)} as any)}/> <select value={c[`${dim}Unit` as const]} onChange={e=>updCrate(c.id,{[`${dim}Unit`]:e.target.value as Unit} as any)}><option value="m">m</option><option value="cm">cm</option></select></p>
              ))}
              ...
              {c.stackable && (<p>On top of <select value={c.stackTargetId??''} onChange={e=>updCrate(c.id,{stackTargetId:e.target.value?Number(e.target.value):undefined})}><option value="">— choose —</option>{availableBases.map(b=><option key={b.id} value={b.id}>{b.label}</option>)}</select></p>)}
              <button onClick={() => setCrates(prev => prev.filter(crate => crate.id !== c.id))}>Delete Crate</button>
            </fieldset>
          );
        })}

        <button onClick={()=>{snapshot(); addCrateFromData({label:`Crate ${crates.length+1}`,length:1,width:1,height:1,weight:50});}}>+ Add blank crate</button>
        <button onClick={undo} disabled={!history.length} style={{marginLeft:8}}>Undo last crate</button>
          ...
          {placed.map(c=>{
            ...
            return (
              <group key={c.id} position={c.position}>
                ...
                <Text position={[0,h/2+0.02,0]} rotation={[-Math.PI/2,0,0]} fontSize={Math.min(l,w)*0.18} color="black" anchorX="center" anchorY="middle">{c.label}</Text>
              </group>
            );
          })}
