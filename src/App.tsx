/* App.tsx – fully‑formed, compile‑safe React‑Three‑Fiber planner with stacking logic */
...
// ─── helpers ──────────────────────────────────────────────────────────
type Unit = 'm' | 'cm';
const toM = (v: number, u: Unit): number => (u === 'cm' ? v / 100 : v);
...
interface Crate {
  ...
  colour: string;
  stackable: boolean;
  stackTargetId?: number; // if undefined → on floor
}
...
// ─── packing algorithm (simple queue with stacking) ──────────────────
function packCrates(truck: Truck, crates: Crate[]): { placed: Placed[]; overflowIds: number[] } {
  const placed: Placed[] = [];
  const overflow: number[] = [];

  const L = toM(truck.length, truck.unit);
  const W = toM(truck.width , truck.unit);
  const H = toM(truck.height, truck.unit);

  let cursorX = 0;

  // First pass – ground crates (no stackTargetId)
  crates.filter(c => !c.stackTargetId).forEach(c => {
    const l = toM(c.length,  c.lengthUnit );
    const w = toM(c.width ,  c.widthUnit  );
...
export default function App() {
  ...
  const overlaps = useMemo(() => {
    ...
    for (let i = 0; i < placed.length - 1; i++) {
      for (let j = i + 1; j < placed.length; j++) {
        ...
        if (a.id === b.stackTargetId || b.id === a.stackTargetId) continue; // intended stack
  ...
  // helpers ----------------------------------------------------------------
  const addCrate = () => setCrates(prev => ([...prev, {
  ...
  return (
    <div style={{ display: 'flex', height: '100vh' }}>
      {overflowIds.length > 0 && <Banner color="#c62828">Overflow: {overflowIds.map(id => crates.find(c => c.id === id)!.label).join(', ')}</Banner>}
